<x-app-layout>
    ...
    <div class="py-6 bg-gray-100 dark:bg-gray-900 min-h-screen transition-colors duration-300">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="bg-red-100 dark:bg-red-900/20 p-4 rounded-lg border border-red-200 dark:border-red-800 transition-colors">
                    <div class="flex items-center">
                        <div class="text-red-600 dark:text-red-400 text-2xl font-bold" id="sesiones-activas-count">
                            {{ $sesionesActivas->count() }}
                        </div>
                        <div class="ml-3 text-red-800 dark:text-red-300">
                            <div class="text-sm font-medium">Sesiones Activas</div>
                        </div>
                    </div>
                </div>
                <div class="bg-green-100 dark:bg-green-900/20 p-4 rounded-lg border border-green-200 dark:border-green-800 transition-colors">
                    <div class="flex items-center">
                        <div class="text-green-600 dark:text-green-400 text-2xl font-bold" id="sesiones-hoy-count">
                            {{ App\Models\Sesion::whereDate('fecha', today())->count() }}
                        </div>
                        <div class="ml-3 text-green-800 dark:text-green-300">
                            <div class="text-sm font-medium">Sesiones Hoy</div>
                        </div>
                    </div>
                </div>
                <div class="bg-yellow-100 dark:bg-yellow-900/20 p-4 rounded-lg border border-yellow-200 dark:border-yellow-800 transition-colors">
                    <div class="flex items-center">
                        <div class="text-yellow-600 dark:text-yellow-500 text-sm font-bold" id="ultima-actualizacion">
                            {{ now()->format('H:i:s') }}
                        </div>
                        <div class="ml-3 text-yellow-800 dark:text-yellow-300">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                        ...
                        <form id="scanner-form" class="space-y-4">
                            ...
                            <div class="flex space-x-2 mb-4">
                                <button type="button" id="btn-manual" class="flex-1 py-2 px-4 bg-blue-500 text-white rounded font-medium transition-colors">
                                    ⌨️ Manual
                                </button>
                                <button type="button" id="btn-camera" class="flex-1 py-2 px-4 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded font-medium transition-colors">
                                    📷 Cámara
                                </button>
                            </div>
                            ...
                            <div id="modo-manual">
                                <div>
                                    <label for="npi" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">
                                        NPI del Alumno
                                    </label>
                                    <input type="text" 
                                           id="npi" 
                                           name="npi" 
                                           class="mt-1 block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 text-lg font-mono bg-white dark:bg-gray-700 text-gray-900 dark:text-white transition-colors"
                            ...
                            <div id="modo-camera" class="hidden">
                                <div class="text-center">
                                    ...
                                    <video id="qr-video" class="w-full max-w-lg mx-auto border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg"></video>
                                    ...
                                    <div id="scanner-status" class="mt-2 text-sm text-gray-600 dark:text-gray-400">
                            ...
                            <div class="mb-6">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    @php
                                        $opciones = [
                                            ['val' => 'Práctica en seco', 'emoji' => '🎭', 'desc' => 'Sin software de simulación'],
                                            ['val' => 'Emergencia en vuelo', 'emoji' => '🚨', 'desc' => 'Procedimientos de emergencia'],
                                            ['val' => 'Trabajo en pista', 'emoji' => '🛫', 'desc' => 'Despegue y aterrizaje'],
                                            ['val' => 'Acrobacias', 'emoji' => '✈️', 'desc' => 'Maniobras acrobáticas'],
                                            ['val' => 'Navegación', 'emoji' => '🧭', 'desc' => 'Vuelo por instrumentos y navegación'],
                                            ['val' => 'Vuelo instrumental', 'emoji' => '📡', 'desc' => 'IFR y aproximaciones'],
                                        ];
                                    @endphp

                                    @foreach($opciones as $opcion)
                                    <label class="flex items-center p-3 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer transition-colors">
                                        <input type="checkbox" name="actividades[]" value="{{ $opcion['val'] }}"
                                               class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded dark:bg-gray-700 dark:border-gray-500">
                                        <span class="ml-3 text-sm text-gray-700 dark:text-gray-300">
                                            <span class="font-medium">{{ $opcion['emoji'] }} {{ $opcion['val'] }}</span>
                                            <span class="block text-xs text-gray-500 dark:text-gray-400">{{ $opcion['desc'] }}</span>
                                        </span>
                                    </label>
                                    @endforeach
                                </div>
                            ...
                            <button type="submit" id="btn-procesar" class="w-full flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-lg font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors">
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg transition-colors duration-300">
                    <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                        ...
                        <div id="sesiones-activas-lista" class="space-y-3 max-h-96 overflow-y-auto pr-1">
                                ...
                                <div class="border border-yellow-200 dark:border-yellow-700/50 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg p-3 transition-colors">
                                    <div class="flex justify-between items-start">
                                        ...
                                        <div class="text-right flex flex-col items-end space-y-2">
                                            <div class="text-sm font-medium text-yellow-700 dark:text-yellow-500">
    ...
    <script>
        ...
        function mostrarResultadoFinalizacion(data) {
            ...
            let html = `<div class="bg-blue-100 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 rounded-lg p-4">`;
            html += `<div class="flex items-start">`;
            html += `<div class="text-2xl mr-3">🏁</div>`;
            html += `<div>`;
            html += `<h4 class="font-medium text-blue-600 dark:text-blue-400">${data.alumno}</h4>`;
            html += `<p class="text-sm text-gray-600 dark:text-gray-300 mt-1">${data.message}</p>`;
            html += `<div class="text-sm text-gray-500 dark:text-gray-400 mt-2">Duración: ${data.duracion}<br>${data.hora_inicio} - ${data.hora_fin}</div>`;
            html += `</div></div></div>`;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            startCamera.addEventListener('click', async function() {
                try {
                    ...
                    qrScanner = new window.QrScanner(video, result => {
                        let npi = result.data;
                        if (/^\d{8}$/.test(npi)) npi = npi.substring(0, 7) + '-' + npi.substring(7);
            ...
            form.addEventListener('submit', function(e) {
                ...
                .catch(error => { console.error('Error:', error); mostrarError('Error de conexión'); })
                .finally(() => {
